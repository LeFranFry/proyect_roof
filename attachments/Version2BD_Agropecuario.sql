...
CREATE TABLE IF NOT EXISTS `mydb`.`Empleado` (
  ...
  `Credenciales_identificacion` INT NOT NULL,
  PRIMARY KEY (`Identificacion`),
  INDEX `fk_Empleado_Credenciales1_idx` (`Credenciales_identificacion` ASC),
  CONSTRAINT `fk_Empleado_Credenciales1`
    FOREIGN KEY (`Credenciales_identificacion`)
    REFERENCES `mydb`.`Credenciales` (`identificacion`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `mydb`.`Hierro` (
  ...
  `Imagen` VARCHAR(45) GENERATED ALWAYS AS () VIRTUAL,
